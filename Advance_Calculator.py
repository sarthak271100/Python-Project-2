...
def while_talk():
    with sr.Microphone() as source:
        ...
        try:
            recognised_text = r.recognize_google(audio_text)
            return display.insert(0,recognised_test)
        except:
            sorry = "Sorry, I did not get that"
            display.insert(0,sorry)
...
Button(root,text="Talk First Operand",command= lambda :while_talk()).grid(columnspan=6, sticky=N+S+E+W)
